...
export function OsmMiniMap({ lat, lng, polygonGeoJson = null, height = '200px', label = '' }) {
  ...
  const loadAllPolygons = useCallback(async (map) => {
    try {
      ...
      if (allPolygonsLayerRef.current) allPolygonsLayerRef.current.remove();
      ...
      console.warn('MiniMap polygons failed:', e.message);
    }
  }, []);
  ...
  }, [lat, lng, polygonGeoJson, loadAllPolygons]);
