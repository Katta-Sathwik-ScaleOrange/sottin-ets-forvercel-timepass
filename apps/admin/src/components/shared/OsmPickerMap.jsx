 ...
 *  • All building footprints (polygons) loaded from API and rendered in green
...
export function OsmPickerMap({
  ...
  const allPolygonsLayerRef = useRef(null);
  ...
  const showPolygon = useCallback((map, item) => {
    ...
    polygonRef.current = L.geoJSON(
      ...
      { style: { color: '#22c55e', weight: 3, opacity: 1, fillColor: '#22c55e', fillOpacity: 0.3 } }
    ).addTo(map);
    try {
  ...
  const loadAllPolygons = useCallback(async (map) => {
    try {
      const { features } = await api.get('/apartments/all-polygons');
      if (allPolygonsLayerRef.current) {
        allPolygonsLayerRef.current.remove();
      }
      allPolygonsLayerRef.current = L.geoJSON(features, {
        style: {
          color: '#22c55e',
          weight: 1.5,
          opacity: 0.6,
        ...
        onEachFeature: (feature, layer) => {
          ...
          layer.on('click', (e) => {
             // Handle polygon click as selection if needed
             // L.DomEvent.stopPropagation(e);
          });
        }
      }).addTo(map);
    } catch (e) {
      console.warn('Could not load all polygons:', e.message);
    }
  }, []);
  ...
  const searchNearPin = useCallback(async (map, lat, lng) => {
    ...
    try {
      ...
      let nearby = [];
      if (mode === 'apartment') {
        nearby = data.match ? [data.match] : (data.nearby || []);
      } else {
  ...
  }, [locations, selectedItem, renderMarkers]);
    ...
    <div className="flex flex-col rounded-2xl overflow-hidden border border-surface-border bg-surface-1"
      ...
      <div className="flex items-center justify-between px-3 py-2 bg-surface-2 border-b border-surface-border flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          >
            {gpsLoading ? <Spinner size="sm" /> : <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>}
        ...
        <button onClick={onClose} className="w-7 h-7 flex items-center justify-center rounded-lg text-slate-400 hover:text-white hover:bg-surface-3 transition-colors">
      ...
      <div className="px-3 py-1.5 bg-surface-2/60 border-b border-surface-border sm:hidden flex-shrink-0">
        <p className="text-slate-500 text-xs truncate">{pinSearching ? '🔍 Searching…' : hint}</p>
      </div>
      ...
      <div className="relative flex-1 min-h-0">
        ...
        <div className="absolute bottom-8 left-2 z-[500] pointer-events-none">
          <div className="bg-black/60 backdrop-blur-sm text-[9px] text-slate-400 px-2 py-1 rounded-lg border border-white/10 space-y-0.5">
            <div className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-green-400 inline-block flex-shrink-0"/><span>{mode === 'apartment' ? 'Apartment' : 'Office'} locations</span></div>
            <div className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-amber-400 inline-block flex-shrink-0"/><span>Dropped pin</span></div>
          </div>
        </div>
      </div>
        ...
        {selectedItem ? (
          <>
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              >Clear</button>
              <button onClick={() => onSelect(selectedItem)} className="bg-brand-500 hover:bg-brand-400 text-white text-sm font-semibold px-4 py-2 rounded-xl transition-colors active:scale-95">Confirm ✓</button>
            </div>
          </>
        ) : (
          <p className="text-slate-500 text-xs w-full text-center">
            {locations.length > 0 ? `${locations.length} location${locations.length > 1 ? 's' : ''} shown — tap a marker to select` : 'Tap the map or use "Locate me" to find nearby locations'}
          </p>
        )}
      </div>
    </div>
